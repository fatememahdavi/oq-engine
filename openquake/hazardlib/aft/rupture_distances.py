...
def get_close_source_pairs(
    ...
    source_pair_dists = [
        (source_list[i].id, source_list[j].id, dist)
        for i, j, dist in source_pair_dists
    ]
...
def stack_xyz_sequences(sequences: Sequence) -> Tuple[np.ndarray]:
    """
...
def split_rows(
    ...
    :returns:
       ...
       `xyz_stack` item split from `stacked_array`, and a `row_idxs` item
       that gives the boundaries of each block within the `xyz_stack`.
    """
    ...
    for i in range(len(closest_first_inds)):
        ...
        if i < len(closest_first_inds) - 1:
            ...
            this["xyz_stack"] = stacked_array[row_ids[start] : row_ids[stop], :]
            ...
            this["xyz_stack"] = stacked_array[row_ids[start] :, :]
            this["row_idxs"] = block_idx - block_idx[0]
...
def get_sequence_pairwise_dists(seq_1, seq_2, max_block_ram=20.0):
    """
    Only used in source to source distance calcs, rather than for rupture
    distance calculations at this point
    """
    r_id_1, xyz_stack_1 = stack_xyz_sequences(seq_1)
    r_id_2, xyz_stack_2 = stack_xyz_sequences(seq_2)

    # breakpoint()

    if xyz_stack_1.shape[0] * xyz_stack_2.shape[0] * 8 < max_block_ram * 1e9:
        dists = calc_pairwise_distances(xyz_stack_1, xyz_stack_2)
        ...
        n_splits = int(
            np.ceil(
                (xyz_stack_1.shape[0] * xyz_stack_2.shape[0] * 8)
                / (max_block_ram * 1e9)
            )
        )
        row_splits = split_rows(r_id_1, xyz_stack_1, n_splits)
        print(n_splits, "splits")

        for start_rup, row_split in row_splits.items():
            # breakpoint()
            dists = calc_pairwise_distances(row_split["xyz_stack"], xyz_stack_2)
...
def get_and_filter_sequence_pairwise_dists(
    seq_1,
    seq_2,
    seq_1_mags,
    dist_constant=4.0,
    ...
    if xyz_stack_1.shape[0] * xyz_stack_2.shape[0] * 8 < max_block_ram * 1e9:
        ...
        min_rup_dists = filter_dists_by_mag(
            min_rup_dists, seq_1_mags.values, dist_constant=dist_constant
        )
        ...
        row_splits = split_rows(r_id_1, xyz_stack_1, n_splits)
        logging.info(f"{n_splits} splits")
        ...
        for start_rup, row_split in row_splits.items():
            ...
            logging.info(f"  {n_split}")

            dists = calc_pairwise_distances(row_split["xyz_stack"], xyz_stack_2)

            these_min_rup_dists = get_min_rup_dists(
                dists, row_split["row_idxs"], r_id_2, row_offset=start_rup
            )

            these_min_rup_dists = filter_dists_by_mag(
                these_min_rup_dists,
                seq_1_mags.values,
                dist_constant=dist_constant,
            )

            min_rup_dist_list.append(these_min_rup_dists)
...
def get_and_filter_sequence_pairwise_dists_nosplit(
    xyz_stack_0,
    xyz_stack_1,
    r_id_0,
    r_id_1,
    ...
    dists = calc_pairwise_distances(xyz_stack_0, xyz_stack_1)
    min_rup_dists = get_min_rup_dists(dists, r_id_0, r_id_1)
    min_rup_dists = filter_dists_by_mag(
        min_rup_dists, seq_0_mags.values, dist_constant=dist_constant
    )

    if isinstance(rup_adj_dict, dict):
        if src_id_0 not in rup_adj_dict.keys():
            rup_adj_dict[src_id_0] = {}
    else:
        if src_id_0 not in rup_adj_dict.keys():
            rup_adj_dict.create_group(src_id_0)

    rup_adj_dict[src_id_0][src_id_1] = min_rup_dists


def get_and_filter_sequence_pairwise_dists_split(
    xyz_stack_0,
    xyz_stack_1,
    r_id_0,
    r_id_1,
    ...
    n_splits = int(
        np.ceil(
            (xyz_stack_0.shape[0] * xyz_stack_1.shape[0] * 8)
            / (max_block_ram * 1e9)
        )
    )
    row_splits = split_rows(r_id_0, xyz_stack_0, n_splits)
    # logging.info(f"{n_splits} splits")
    ...
    for start_rup, row_split in row_splits.items():
        ...
        dists = calc_pairwise_distances(row_split["xyz_stack"], xyz_stack_1)

        these_min_rup_dists = get_min_rup_dists(
            dists, row_split["row_idxs"], r_id_1, row_offset=start_rup
        )

        these_min_rup_dists = filter_dists_by_mag(
            these_min_rup_dists, seq_0_mags.values, dist_constant=dist_constant
        )

        if h5_flag:
            write_rups_dist_chunks_to_hdf5(
                src_id_0, src_id_1, these_min_rup_dists, rup_adj_dict
            )
        else:
            min_rup_dists.append(these_min_rup_dists)

    if not h5_flag:
        min_rup_dists = np.hstack(min_rup_dists)
        if src_id_0 not in rup_adj_dict.keys():
            rup_adj_dict[src_id_0] = {}
        rup_adj_dict[src_id_0][src_id_1] = min_rup_dists
...
def get_rup_dist_pairs(
    ...
    min_rup_dists = get_and_filter_sequence_pairwise_dists(
        rups1.xyz, rups2.xyz, rups1.mag, max_block_ram=max_block_ram
    )
...
def process_source_pair(
    ...
    src_id_0 = str(source_id_0)
    src_id_1 = str(source_id_1)

    rups0 = rup_df.iloc[source_groups.groups[source_id_0]]
    rups1 = rup_df.iloc[source_groups.groups[source_id_1]]

    r_id_0, xyz_stack_0 = stack_xyz_sequences(rups0.xyz)
    r_id_1, xyz_stack_1 = stack_xyz_sequences(rups1.xyz)

    # breakpoint()

    if xyz_stack_0.shape[0] * xyz_stack_1.shape[0] * 8 < max_block_ram * 1e9:
        get_and_filter_sequence_pairwise_dists_nosplit(
            xyz_stack_0=xyz_stack_0,
            xyz_stack_1=xyz_stack_1,
            r_id_0=r_id_0,
            r_id_1=r_id_1,
            seq_0_mags=rups0.mag,
            dist_constant=dist_constant,
            src_id_0=src_id_0,
            src_id_1=src_id_1,
            rup_adj_dict=rup_adj_dict,
        )

    else:
        get_and_filter_sequence_pairwise_dists_split(
            xyz_stack_0=xyz_stack_0,
            xyz_stack_1=xyz_stack_1,
            r_id_0=r_id_0,
            r_id_1=r_id_1,
...
def write_rups_dist_chunks_to_hdf5(
    src_id_0, src_id_1, min_rup_dist_chunk, h5_dict
):

    if src_id_0 not in h5_dict.keys():
        h5_dict.create_group(src_id_0)

    if src_id_1 not in h5_dict[src_id_0].keys():
        h5_dict[src_id_0].create_dataset(
            src_id_1,
            data=min_rup_dist_chunk,
            compression="gzip",
            chunks=True,
            maxshape=(None,),
        )
    else:
        h5_dict[src_id_0][src_id_1].resize(
            (
                h5_dict[src_id_0][src_id_1].shape[0]
                + min_rup_dist_chunk.shape[0]
            ),
            axis=0,
        )
        h5_dict[src_id_0][src_id_1][
            -min_rup_dist_chunk.shape[0] :
        ] = min_rup_dist_chunk
...
def prep_source_data(
    ...
    for i, source in enumerate(tqdm(sources, leave=False)):
        ...
        for j, r in enumerate(rup_list):
            r.source = source.id
            source_rup_inds.append((r.source, j))
            # r.source = i
...
def get_aft_rup_dists(sources, h5_file=None, 
                      rup_df=None,
                      source_groups=None,
                      dist_constant: float=4.0, 
                      max_block_ram: float=20.):
